...
import { csvCell, extensionKind, type SearchResult } from '@/lib/domains';
...
function activeCount(value: number | null | undefined, total: number) {
  return typeof value === 'number' && Number.isSafeInteger(value) && value >= 0 && value <= total ? value : null;
}
...
export function SearchResults({ result, onSelectName }: { result: SearchResult; onSelectName: (name: string) => void }) {
  ...
  function exportResults() {
    const exported: (string | number | null)[][] = view === 'overview'
      ? [['Name', 'Match', 'Count', 'Active websites', 'Extensions shown', 'Data source', 'Filtered'],
        ...rows.map(row => [row.name, row.exact ? 'Exact' : 'Related', row.count,
          activeCount(row.activeCount, row.count) ?? 'Not checked',
          row.suffixes.filter(matchesFilter).map(suffix => `.${suffix}`).join(' '), sourceLabel, hasFilter ? 'Yes' : 'No'])]
      : [['Keyword', 'Domain', 'Extension', 'Type', 'Data source'],
        ...exactSuffixes.map(suffix => [result.query, `${result.query}.${suffix}`, `.${suffix}`, extensionKind(suffix), sourceLabel])];
    const blob = new Blob(['\uFEFF' + exported.map(row => row.map(csvCell).join(',')).join('\r\n')], { type: 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `regcount-${result.query}-${view}-${result.source}.csv`;
    link.click();
  ...
  return <>
    ...
    <dl className="results-metrics" aria-label="Search summary">
      <div className="metric-primary"><dt>Exact-match count</dt><dd>{total.toLocaleString()}</dd><dd className="metric-detail">Extensions for {result.query}</dd></div>
    ...
    <div className="results-coverage">
      <span><strong>{result.suffixes.length.toLocaleString()}</strong> exact extensions returned</span>
      ...
      {result.suffixes.length < total && <span className="coverage-warning">The source returned a partial extension list.</span>}
    </div>
    <Tabs value={view} onValueChange={setView} className="result-tabs">
          ...
          <tbody role="rowgroup">{rows.map(row => {
            ...
            return <tr key={row.name} role="row" className={row.exact ? 'exact-result-row' : undefined}>
              <th scope="row" role="rowheader" className="result-name-cell">
                ...
                <span className={row.exact ? 'exact-match-label' : 'related-match-label'}>{row.exact ? 'Exact match' : 'Related name'}</span>
              </th>
              <td role="cell" className="result-count-cell"><span className="mobile-column-label" aria-hidden="true">Count</span><strong>{row.count.toLocaleString()}</strong></td>
              ...
              <td role="cell" className="result-extensions-cell">
                ...
                {!suffixes.length && <p className="row-extension-note">{hasFilter ? 'No extensions match this filter.' : row.count > 0 ? 'Extension list not supplied.' : 'No extensions returned.'}</p>}
                ...
                {row.suffixes.length < row.count && <p className="row-extension-note">{row.suffixes.length} of {row.count.toLocaleString()} extensions supplied by the source.</p>}
              </td>
            </tr>;
          })}</tbody>
        </table>
      ...
      <TabsContent value="extensions">
        ...
        {!exactSuffixes.length && <div className="filter-empty">{hasFilter ? 'No extensions match this filter.' : 'No extensions returned for this name.'}</div>}
