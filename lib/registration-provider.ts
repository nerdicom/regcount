...
async function queryProvider(query:string,key:string,position:SearchPosition,exact=false){
 const url=new URL('https://api.dotdb.com/v2/search');url.searchParams.set('keyword',query);url.searchParams.set('position',position);if(exact){url.searchParams.set('min_length',String(query.length));url.searchParams.set('max_length',String(query.length));}
...
export async function searchRegistrations(value:string,requestedPosition:unknown='any'):Promise<SearchResult>{
 ...
 const payload=await queryProvider(query,key,position);
 ...
 if(total>0&&!exact){const exactPayload=await queryProvider(query,key,position,true);exact=parse(exactPayload.matches).find(row=>row.name===query);}
 ...
 const related=matches.filter(row=>row.name!==query&&matchesPosition(row.name,query,position));
 const result:SearchResult={query,position,source:'dotdb',total,suffixes:exact?.suffixes||[],related,relatedPartial:Number(payload.total_name)>matches.length||matches.some(row=>!matchesPosition(row.name,query,position)),fetchedAt:new Date().toISOString()};
 if(cache.size>=200)cache.delete(cache.keys().next().value!);cache.set(cacheKey,{expires:Date.now()+5*60*1000,value:result});return result;
}
